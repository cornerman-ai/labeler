// chin_shoulder.js — chin-shoulder labeler 2.0.
//
...
//
// The order lives in queue.json, baked once and identical for everybody (see
// chin_build_queue.py). Position is derived from the labeler's own saved rows,
// never stored client-side, so any browser resumes in the same place.
//
// Everything the page draws comes from queue.json. It never loads a video, and
// never runs pose detection — the five joints and the chin proxy are baked.
...
const FIELDS = ['shoulder_ok', 'chin_ok', 'lateral_safe', 'frontal_safe'];
...
const state = {
  ...
  mode: localStorage.getItem('csMode2') === 'reviewer' ? 'reviewer' : 'active',
...
// "Finished" = every question answered. A skip is not finished, and neither is
// a partially answered frame — both are things still to come back to, so they
// must not inflate the count. Same predicate the overview paints green with,
// so the number and the dots cannot drift apart.
...
async function loadTeam() {
  let rows;
  try {
    rows = (await call({ action: 'statsChinShoulderV2' }, 'team')).labelers;
  } catch (e) {
...
async function fetchRanges(labeler, n) {
  ...
  try {
    if (labeler.toLowerCase() === (who() || '').toLowerCase()) {
      ...
      const body = await call({ action: 'listChinShoulderV2', labeler }, 'frames');
      rows = body.rows || [];
    }
  } catch (e) {
...
function setMode(mode) {
  state.mode = mode;
  localStorage.setItem('csMode2', mode);
  renderMode();
}
function renderMode() {
...
function bind() {
  ...
  const KEYS = {
    '1': ['shoulder_ok', 'yes'], '2': ['shoulder_ok', 'no'],
    q: ['chin_ok', 'yes'], w: ['chin_ok', 'no'],
    a: ['lateral_safe', 'yes'], s: ['lateral_safe', 'no'], d: ['lateral_safe', 'hard_to_say'],
    z: ['frontal_safe', 'yes'], x: ['frontal_safe', 'no'], c: ['frontal_safe', 'hard_to_say'],
  };
...
(async function init() {
  ...
  bind();
  try {
    const res = await fetch(`queue.json?v=8`);

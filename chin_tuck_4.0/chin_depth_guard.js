// chin_depth_guard.js — chin-point labeler 4.0, DEPTH-GUARD variant.
//
// GEOMETRY, not a verdict — same idea as height-guard (chin_tuck4.js), same
...
// per point.
//
// v1: reuses height-guard's frame set AS-IS (depth_queue.json is literally
// the first 200 slots of chin_tuck_4.0/queue.json) rather than waiting on a
// real sideways-only sampling gate — that gate is still TODO. So some frames
...
// and depth_queue.json. See cornerman-backend ml/research/chin_tuck/v3/.
//
...
const state = {
  frames: [],              // depth_queue.json order (a straight slice of height-guard's queue)
...
(async function init() {
  ...
  bind();
  try {
    const res = await fetch('depth_queue.json?v=1');
    ...
    const msg = viaFile
      ...
      : 'Could not load depth_queue.json (' + e.message + ').';
    setReady(false, msg, true);
    status(msg, 'err');
    return;
  }
  if (!state.frames.length) {
    setReady(false, 'depth_queue.json loaded but contains no frames.', true);
    status('depth_queue.json is empty', 'err');
    return;
  }

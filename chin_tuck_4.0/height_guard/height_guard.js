...
// it is height-guard-specific.
const MODE_AXES = ['height', 'depth'];
const MODE_FOLD_KEY = 'cs4_mode_folded';
...
async function loadRoster(force) {
  const params = { action: 'statsChinPoint' };
...
async function pingPresence() {
  ...
  if (!f) return;
  try {
    await call({ action: 'pingPresenceChinPoint', video: f.stem, round: String(f.round), frame: String(f.frame) }, 'presence');
  } catch (e) { /* the next tick tries again; not worth a status-line error */ }
}

async function pollPresence() {
  try {
    const body = await call({ action: 'getPresenceChinPoint' }, 'presence');
...
const BUG_REPORT_TOOL = 'height_guard';
...
function exportDisagreementPNG() {
  ...
  canvas.toBlob((blob) => {
    ...
    link.href = url;
    link.download = `disagreement-height_guard-${(a || 'a').toLowerCase()}-${(b || 'b').toLowerCase()}-${stampFile}.png`;
    link.click();
...
function exportModePNG() {
  ...
  canvas.toBlob((blob) => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `alignment-height_guard-${(a || 'a').toLowerCase()}-${(b || 'b').toLowerCase()}-${stampFile}.png`;
    link.click();
    URL.revokeObjectURL(url);
  }, 'image/png');
}

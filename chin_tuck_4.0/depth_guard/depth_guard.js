...
const AGREE_PAIR_KEY = 'cs4d_agree_pair';
...
// correctly on height_guard/height_impact (where the second point is the
// shoulder TOP) exactly as it does here (the shoulder's FRONTAL point) —
// nothing about it is depth-guard-specific.
const MODE_AXES = ['height', 'depth'];
const MODE_FOLD_KEY = 'cs4d_mode_folded';
...
const BUG_REPORT_TOOL = 'depth_guard';
...
function exportModePNG() {
  ...
  canvas.toBlob((blob) => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `alignment-depth_guard-${(a || 'a').toLowerCase()}-${(b || 'b').toLowerCase()}-${stampFile}.png`;
    link.click();
    URL.revokeObjectURL(url);
  }, 'image/png');
}

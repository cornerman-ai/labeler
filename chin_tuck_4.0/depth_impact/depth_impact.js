// depth_impact.js — chin-point labeler 4.0, DEPTH-IMPACT variant.
//
// GEOMETRY, not a verdict — same idea as height_impact, same two-point-click
// machinery, different landmark pair: click the chin tip, then the MOST
// FRONTAL point of the punching-hand shoulder (front-to-back, not
...
// two points, same as height_impact; disagreement is a distance, diagnosable
// per point.
//
// depth_guard/height_guard sample the non-punch, punch-adjacent GUARD band.
// This variant is the depth half of the impact side of the four-way matrix:
// every frame here IS the human-labeled IMPACT frame of a real punch (Impact
// Frames sheet, joined to Combined Data on punch_uuid), further filtered to
// angle==Side directly off that join — no borrowed-nearest-punch angle
// inference the way depth_guard's non-punch frames need, since every impact
// frame already has its own punch row to read the angle off. Because every
// sample is tied to one punch_uuid, the shoulder to mark is known EXACTLY —
// the PUNCHING hand's shoulder (lead for a lead-hand punch, rear for a
// rear-hand one) — not inferred from stance. `shoulder` on each queue frame
...
// project bucket behind a shared download token. Unlike depth_guard, this
// variant's queue.json is ALSO Firebase-only — git carries only this page's
// code, nothing for the data. UPLOADED UNDER ITS OWN PREFIX
// (labeler_media/chin_tuck/v4/depth_impact_v4_frames/frames, not
// height_impact's .../height_impact_v4_frames/frames) — see FRAME_PREFIX and
// QUEUE_URL below — because this sample's frame set is a genuinely different
// pool (side-view-filtered impact frames), not a slice of height_impact's.
//
...
//
// REPEATS: 184 planted (rep=1, ~10% of the 1,842 real frames, repeat_gap=200
// slots apart, blind), same contract as height_impact's — the noise floor
// inter-rater numbers get read against. rep is part of the row identity
// (key()/rowKey()) so a repeat never overwrites its original.
//
// The backend OVERWRITES IN PLACE (saveChinPointDepthImpact), same as
// 2.0/3.0: a save for a (video,round,frame,rep) that already has a row
// replaces it, and only a new identity appends. A re-label is not kept as
// separate history.
//
...
// Own prefix, NOT height_impact's labeler_media/chin_tuck/v4/
// height_impact_v4_frames/frames — this sample is a genuinely different
// (side-view-filtered) pool of impact frames, not a slice of height_impact's,
// so it was uploaded under its own variant directory. See the file header
// and chin_upload_frames.py.
const FRAME_PREFIX = 'labeler_media/chin_tuck/v4/depth_impact_v4_frames/frames';
const FRAME_TOKEN = '628dbeba-2969-4f45-b65e-5b295ef56fdc';

// Unlike depth_guard, this variant's queue.json is NOT committed to this
// repo — it lives next to the frames in Firebase Storage
// (labeler_media/chin_tuck/v4/depth_impact_v4_frames/queue.json), uploaded
// straight from cornerman-backend without a git round-trip. Bump
// QUEUE_CACHE_BUST when the backend regrows the queue, same purpose as the
// old '?v=N' on the committed file.
const QUEUE_CACHE_BUST = 1;
const QUEUE_URL = 'https://firebasestorage.googleapis.com/v0/b/' + FRAME_BUCKET + '/o/'
  + encodeURIComponent('labeler_media/chin_tuck/v4/depth_impact_v4_frames/queue.json')
  + `?alt=media&token=${FRAME_TOKEN}&cb=${QUEUE_CACHE_BUST}`;
...
const HIDE_KEY = 'cs4di_hidden';        // localStorage: names hidden from MY OWN team list
const RANGE_KEY = 'cs4di_ranges';       // localStorage: cached per-labeler frame ranges
...
const THRESH_KEY_CHIN = 'cs4di_agree_thresh_chin';
const THRESH_KEY_SH = 'cs4di_agree_thresh_sh';
...
// reaches the sheet.
const FOLD_KEY = 'cs4di_metric_folded';
...
const AGREE_PAIR_KEY = 'cs4di_agree_pair';
...
// correctly on the height variants (where the second point is the shoulder
// TOP) exactly as it does here (the shoulder's FRONTAL point) — nothing
// about it is depth-impact-specific.
const MODE_AXES = ['height', 'depth'];
const MODE_FOLD_KEY = 'cs4di_mode_folded';
...
const state = {
  ...
  failed: new Map(),
  ready: false,
  camBad: false,           // camera angle too frontal to judge depth on THIS frame
  shownAt: 0,
...
// One retry for cold-start blips; the v4di marker (CS4DI_SPEC's tag) refuses
// a deployment that predates these endpoints (doGet answers unknown
// actions with a success shape, so without the marker a save could
...
async function loadLabels() {
  ...
  if (!name) return;
  const body = await call({ action: 'listChinPointDepthImpact', labeler: name }, 'load labels');
...
async function loadRoster(force) {
  const params = { action: 'statsChinPointDepthImpact' };
...
async function pingPresence() {
  ...
  if (!f) return;
  try {
    await call({ action: 'pingPresenceChinPointDepthImpact', video: f.stem, round: String(f.round), frame: String(f.frame) }, 'presence');
  } catch (e) { /* the next tick tries again; not worth a status-line error */ }
}

async function pollPresence() {
  try {
    const body = await call({ action: 'getPresenceChinPointDepthImpact' }, 'presence');
...
async function fetchRanges(labeler, n) {
  ...
  try {
    if (labeler.toLowerCase() === (who() || '').toLowerCase()) {
      ...
      const body = await call({ action: 'listChinPointDepthImpact', labeler }, 'frames');
      rows = body.rows || [];
    }
  } catch (e) {
...
const BUG_REPORT_TOOL = 'depth_impact';
...
async function loadTeamRows() {
  ...
  const results = await Promise.allSettled(state.roster.map(async (l) => {
    const body = await call({ action: 'listChinPointDepthImpact', labeler: l.labeler }, `load ${l.labeler}`);
    const rows = new Map();
...
// for ONE metric — not an aggregate. The admin is looking at a specific frame;
...
function saveTeammateRow(labeler) {
  ...
  const params = {
    action: 'saveChinPointDepthImpact', labeler,
...
function save({ skip = null } = {}) {
  ...
  const params = {
    action: 'saveChinPointDepthImpact', labeler: name,
...
//
// Disabled until both points are placed: "too frontal" is a claim about
...
function exportDisagreementPNG() {
  ...
  canvas.toBlob((blob) => {
    ...
    link.href = url;
    link.download = `disagreement-depth_impact-${(a || 'a').toLowerCase()}-${(b || 'b').toLowerCase()}-${stampFile}.png`;
    link.click();
...
function exportModePNG() {
  ...
  canvas.toBlob((blob) => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `alignment-depth_impact-${(a || 'a').toLowerCase()}-${(b || 'b').toLowerCase()}-${stampFile}.png`;
    link.click();
    URL.revokeObjectURL(url);
  }, 'image/png');
}
...
(async function init() {
  ...
  try {
    ...
    const msg = 'Could not load the impact queue from Firebase Storage (' + e.message + ').';
    setReady(false, msg, true);
    status(msg, 'err');
    return;
  }
  if (!state.frames.length) {
    setReady(false, 'Impact queue loaded but contains no frames.', true);
    status('Impact queue is empty', 'err');
    return;
  }

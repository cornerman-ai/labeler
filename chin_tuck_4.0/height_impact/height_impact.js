// height_impact.js — chin-point labeler 4.0, HEIGHT-IMPACT variant.
//
// GEOMETRY, not a verdict — same idea as height_guard/depth_guard: click the
// chin tip, click the top of the shoulder. Over/level/under is DERIVED
// downstream from the two points; disagreement becomes a distance in pixels,
// diagnosable per point; and the clicks double as calibration for the
// pipeline (human shoulder vs BlazePose's, human chin vs the nose→mouth
// extrapolation).
//
// height_guard/depth_guard sample the non-punch, punch-adjacent band — the
// GUARD position, between punches. This variant is the opposite half of the
// four-way matrix: every frame here IS the human-labeled IMPACT frame of a
// real punch (Impact Frames sheet, joined to Combined Data on punch_uuid),
// so it measures the chin/shoulder relationship at the moment that matters
// most, not the position either side of it. Because every sample is tied to
// one punch_uuid, the shoulder to mark is known EXACTLY — the PUNCHING
// hand's shoulder (lead for a lead-hand punch, rear for a rear-hand one),
// not inferred from stance the way the guard variants' hint is. `shoulder`
// on each queue frame already carries that answer; there is nothing
// stance-derived left for this page to guess at.
//
...
// project bucket behind a shared download token. Unlike height_guard/
// depth_guard, this variant's queue.json is ALSO Firebase-only — git carries
// only this page's code, nothing for the data. See QUEUE_URL below and
// cornerman-backend ml/research/chin_tuck/v3/.
//
...
const FRAME_PREFIX = 'labeler_media/chin_tuck/v4/height_impact_v4_frames/frames';
const FRAME_TOKEN = '628dbeba-2969-4f45-b65e-5b295ef56fdc';

// Unlike height_guard/depth_guard, this variant's queue.json is NOT committed
// to this repo — it lives next to the frames in Firebase Storage
// (labeler_media/chin_tuck/v4/height_impact_v4_frames/queue.json), since the
// impact sample was built and uploaded straight from cornerman-backend
// without a git round-trip. Bump QUEUE_CACHE_BUST when the backend regrows
// the queue, same purpose as the old '?v=N' on the committed file.
const QUEUE_CACHE_BUST = 1;
const QUEUE_URL = 'https://firebasestorage.googleapis.com/v0/b/' + FRAME_BUCKET + '/o/'
  + encodeURIComponent('labeler_media/chin_tuck/v4/height_impact_v4_frames/queue.json')
...
const HIDE_KEY = 'cs4i_hidden';        // localStorage: names hidden from MY OWN team list
const RANGE_KEY = 'cs4i_ranges';       // localStorage: cached per-labeler frame ranges
...
const THRESH_KEY_CHIN = 'cs4i_agree_thresh_chin';
const THRESH_KEY_SH = 'cs4i_agree_thresh_sh';
...
// reaches the sheet.
const FOLD_KEY = 'cs4i_metric_folded';
...
const AGREE_PAIR_KEY = 'cs4i_agree_pair';
...
const state = {
  frames: [],              // height_impact_queue.json order (originals + planted repeats)
...
// One retry for cold-start blips; the v4hi marker (CS4I_SPEC's tag) refuses
...
async function loadLabels() {
  ...
  if (!name) return;
  const body = await call({ action: 'listChinPointImpact', labeler: name }, 'load labels');
...
async function loadRoster(force) {
  const params = { action: 'statsChinPointImpact' };
...
async function pingPresence() {
  ...
  if (!f) return;
  try {
    await call({ action: 'pingPresenceChinPointImpact', video: f.stem, round: String(f.round), frame: String(f.frame) }, 'presence');
  } catch (e) { /* the next tick tries again; not worth a status-line error */ }
}

async function pollPresence() {
  try {
    const body = await call({ action: 'getPresenceChinPointImpact' }, 'presence');
...
async function fetchRanges(labeler, n) {
  ...
  try {
    if (labeler.toLowerCase() === (who() || '').toLowerCase()) {
      ...
      const body = await call({ action: 'listChinPointImpact', labeler }, 'frames');
      rows = body.rows || [];
    }
  } catch (e) {
...
async function loadTeamRows() {
  ...
  const results = await Promise.allSettled(state.roster.map(async (l) => {
    const body = await call({ action: 'listChinPointImpact', labeler: l.labeler }, `load ${l.labeler}`);
    const rows = new Map();
...
function saveTeammateRow(labeler) {
  ...
  const params = {
    action: 'saveChinPointImpact', labeler,
...
function save({ skip = null } = {}) {
  ...
  const params = {
    action: 'saveChinPointImpact', labeler: name,
